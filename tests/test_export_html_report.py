...
def make_scenario(scenario_cls):
    ...
    scenario.security_rules_by_dg = {"dg1": [SecurityRule(name="rule1", action="allow")]}  # type: ignore
    scenario.address_groups = []
    scenario.address_objects = []
    scenario.device_groups = ["dg1"]
    scenario.analysis_results_by_dg = {"dg1": [(scenario.security_rules_by_dg["dg1"][0], [])]}
    return scenario
