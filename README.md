...
A full example HTML report generated by the tool can be found in [`big_report.html`](./tests/big_report.html). This report demonstrates the output for multiple device groups, realistic firewall rules, and shadowing analysis.

You can generate this file by running the test:

```bash
pytest tests/test_export_html_report.py -k big_report
```

Then open `tests/big_report.html` in your browser to view the report.
