...
@click.command()
@model_argument(FakeModel, "models")
def fake_cmd_model(models):
...
def test_model_argument_file_loading(cli_runner, tmp_path):
    ...
    result = cli_runner.invoke(fake_cmd_model, [str(test_file)])
    assert result.exit_code == 0
    assert "Loaded: 1 items" in result.output


def test_model_argument_file_not_found(cli_runner):
    result = cli_runner.invoke(fake_cmd_model, ["missing.json"])
