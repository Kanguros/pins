...
def get_scenario_directories_from_config(config_file: str = "config.yaml") -> list[str]:
    """
    ...
    try:
        ...
        logger.debug(f"Config file '{config_file}' not found, using default scenario directories")
        return []
    except yaml.YAMLError as e:
    ...
    logger.warning(f"Invalid scenarios configuration: expected list or string, got {type(scenarios)}")
    return []
