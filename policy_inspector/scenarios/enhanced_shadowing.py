...
class EnhancedShadowingScenario(Scenario):
    ...
    def __init__(self, panorama: "PanoramaConnector", **kwargs):
        ...
        if self.exclude_checks:
            self.checks = [
                ...
                if not any(
                    keyword in check.__name__ for keyword in self.exclude_checks
                )
            ]
    ...
    def execute(self) -> dict[str, dict[str, dict[str, dict]]]:
        ...
        for device_group in self.device_groups:
            ...
            if not security_rules:
                logger.warning(
                    f"No security rules found in device group: {device_group}"
                )
                continue
            ...
            for rule_index, rule in enumerate(security_rules):
                rule_name = rule.name
                logger.debug(
                    f"Analyzing rule {rule_index + 1}/{len(security_rules)}: {rule_name}"
                )
                ...
                for preceding_rule in preceding_rules:
                    ...
                    for check in self.checks:
                        try:
                            ...
                            logger.error(
                                f"Check {check.__name__} failed for rule {rule_name}: {e}"
                            )
                            check_results[check.__name__] = {
                                "result": False,
    ...
    def get_data_for_display(self) -> dict:
        ...
        for result in self._analysis:
            rule_info = {
                ...
                "shadowing_rules": [
                    sr["name"] for sr in result["shadowing_rules"]
                ],
            }
    ...
    def get_data_for_export(self) -> dict:
        ...
        export_data = {
            ...
            "summary": {
                "total_shadowed_rules": len(self._analysis),
                "device_groups_analyzed": len(
                    {r["device_group"] for r in self._analysis}
                ),
            },
